...
// Telemetry counter for Gemini calls (per process)
...
export async function generateResponse(mensaje, historial = [], contexto = {}, historySummary = null) {
  try {
    ...
    // Obtener config y contexto en paralelo para construir el prompt correctamente
    let clientConfig;
    let relevantContext;
    try {
      [clientConfig, relevantContext] = await Promise.all([
    ...
    // Construir el prompt con el historial mejorado (un único lugar de definición)
    ...
    // Telemetry: tokens de entrada para este prompt (aproximados)
    const reqId = `${Date.now()}_${Math.random().toString(36).slice(2,7)}`;
    const inTokens = estimateTokens(prompt);
    console.log(`🔎 [TELEMETRY] req=${reqId} inTokens≈${inTokens} (prompt) | length=${prompt.length}`);

    // Configuración del modelo
  const modelOptions = {
      model: 'gemini-2.5-flash-lite', // modelo recomendado para respuesta rápida
      generationConfig: {
        temperature: 0.7,
        maxOutputTokens: 800
      }
    ...
    // Debug: log which Gemini model is used for responses
    console.log(`🔧 Gemini model for responses: ${modelOptions.model}`);

    // Debug: log which Gemini model is used for responses
    console.log(`🔧 Gemini model for responses: ${modelOptions?.model}`);

    // Incrementar contador de llamadas a Gemini
    geminiCallCount += 1;
    console.log(`🔧 Gemini calls in flow: ${geminiCallCount}`);
    ...
    // Telemetry: tokens de salida estimados
    const outTokens = estimateTokens(textRes);
    console.log(`🔎 [TELEMETRY] req=${reqId} outTokens≈${outTokens} (response) | length=${textRes.length}`);
    if (result?.usage) {
      console.log('🔧 Gemini usage (raw):', result.usage);
    }
